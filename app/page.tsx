...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/erp", { signal: controller.signal })
      .then(async (r) => {
        const d = (await r.json()) as State & { error?: string };
        if (!r.ok) throw new Error(d.error || "Could not load the workspace");
        return d;
  ...
  useEffect(() => {
    if (!agent) return;
    const controller = new AbortController();
    fetch("/api/mockups?agent=" + encodeURIComponent(agent), {
  ...
  async function generateMockup(e: FormEvent<HTMLFormElement>) {
    ...
    await perform("mockup", async () => {
      const r = await fetch("/api/mockups", { method: "POST", body: payload });
    ...
    <main className="workspace">
      ...
      <header className="topbar">
        ...
        <div className="actions">
          <span className="badge">Local workspace · Oman</span>
          <Choice
            value={agent}
            onChange={(a) => {
              if (draft) {
                  ...
                  <TableBody>
                    {view.lines.map((l, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>{money(l.unitBaisa)}</TableCell>
                        <TableCell>{money(l.quantity * l.unitBaisa)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
                  ...
                  <TableBody>
                    {filtered.slice(page * 30, (page + 1) * 30).map((p) => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell>{p.warehouse_stock}</TableCell>
                        <TableCell>{p.supplier_stock ?? "Unknown"}</TableCell>
                        <TableCell>
        ...
        <TabsContent value="settings">
          <div className="settings-grid">
            ...
            <section className="panel">
              <h2>Sales agents</h2>
              <p className="helper">
                Local profiles organise drafts and mockups. Staff sign-in will
